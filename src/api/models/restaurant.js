const db = require('../../database/dbObject');
    ...
    const deleteIngredients = async (id) => db('INGREDIENTS').where('MENU_ITEM_ID', id).del();
    ...
    const getMenuIngredient = async (id) => db.select('*').table('INGREDIENTS').where({MENU_ITEM_ID: id});
    ...
    const getProvider = async (ids) => db.select('*').table('PROVIDER_INFO').where('ID', ids);

    const getProviders = async (ids) => db.select('*').table('PROVIDER_INFO').whereIn('ID', ids);
    ...
    const updateFeedback = async (orderID, itemID, feedbackObject) => db('FEEDBACK').where({ITEM_ID: itemID, ORDER_ID: orderID}).update(feedbackObject).returning('*');

    const getFeedback = async (providerID, itemID) => db.select('*').table('FEEDBACK').where({PROVIDER_ID: providerID,  ITEM_ID: itemID});

    const getPendingFeedback = async (feedback) => db.raw('SELECT * FROM PUBLIC."FEEDBACK" WHERE "USER_ID" = :userID AND "ORDER_ID" = (SELECT "ORDER_ID" FROM PUBLIC."FEEDBACK" WHERE "USER_ID"=:userID AND "RATING" IS NULL GROUP BY "ORDER_ID" limit 1)', feedback);
