...
class RestaurantService extends BaseService{
    ...
    async createRestaurant(data) {
        try{
            ...
            if(!isEmpty(data.image)){
                const imageWithType = data.image.split(",", 2);
                restaurantObject.IMAGE_TYPE = imageWithType[0] + ",";
                restaurantObject.IMAGE=Buffer.from(imageWithType[1], 'base64');
            }
    ...
    async getNearbyRestaurants(data) {
        try{
            ...
            let restaurants = await restaurantModel.getNearbyRestaurants({lat: data.latitude, lng: data.longitude});
            ...
            restaurants.rows.forEach(x => {
                ...
                if (x.IMAGE) x.IMAGE = x.IMAGE_TYPE + Buffer.from(x.IMAGE, 'binary').toString('base64');
            });

            if (!isEmpty(data.catId)) {
                restaurants = restaurants.rows.filter(x => x.CATEGORY_ID === data.catId);
                return restaurants;
            }
    ...
    async getBestSellingRestaurants(data) {
        try{
            ...
            const URL = 'http://localhost:8080/tms/oms/order/getBestSellerProviderIds';

            const axiosResponse = await axios.get(URL);
            const responseObject = axiosResponse.data;
            console.log(">>>>>>>>>>>>>>responseObject<<<<<<<<<<<<<<<<",responseObject.result)
            let restaurants = await restaurantModel.getNearbyRestaurants({lat: data.latitude, lng: data.longitude});
            ...
            restaurants = restaurants.rows.filter(item => responseObject.result.indexOf(item.id) !== -1);
    ...
    async getPastRestaurants(data) {
        try{

            //checks if the request body contains all the required parameters
            const [paramsValidated, err] = this.restaurantUtils.validateGetPastParams(data)
            if (!paramsValidated){
                throw new Exception(STATUS_CODES.BAD_REQUEST, err)
            }

            const URL = `http://localhost:8080/tms/oms/order/getYourRestaurants/${data.userID}`;
            ...
            let restaurants = await restaurantModel.getProviders(responseObject.result);
            // console.log(">>>>>>>>>>>>>>>>>>restaurant<<<<<<<<<<<<<<<<<<<<<<",restaurants)
    ...
    async getRestaurantMenu(data) {
        try{
            ...
            for (let x of menu) {
                ...
                x.OPTIONAL_INGREDIENTS = ingredients.filter(y => y.IS_OPTIONAL === 1).map(z => z.INGREDIENT_NAME);
    ...
    async createMenu(data) {
        ...
        try{
            ...
            for (const x of data) {
                ...
                if(!isEmpty(x.image)){
                    const imageWithType = x.image.split(",", 2);
                    menuObject.IMAGE_TYPE = imageWithType[0] + ",";
                    menuObject.IMAGE=Buffer.from(imageWithType[1], 'base64');
                }
            ...
            console.log(">>>>>>>>>>>>> Menu Created: ", menuItems[0]);
    ...
    async addIngredients(data) {
        try{
            ...
            if (!isEmpty(data.optionalIngredients)) {
                data.optionalIngredients.forEach(x => {
            ...
            //Executes the insert query to insert a restaurant
            //A list of all objects that are inserted is returned
            await restaurantModel.deleteIngredients(data.itemID);
    ...
    async getAllCategories() {
        try{
            ...
            const categories = await restaurantModel.getAllCategories();
    ...
    async getQueueState(data) {
        try{
            ...
            if (currentQueueSize.CURRENT_QUEUE_SIZE != 0) {
                let obj = {};
                obj.CURRENT_QUEUE_SIZE =  currentQueueSize.CURRENT_QUEUE_SIZE-1;
                const changedObject = await restaurantModel.updateRestaurant(data.providerID, obj);
                return true;
            }
            return false;
    ...
    async getFeedback(data) {
        try{
            ...
            console.log('USMAN1', ids);
            console.log('USMAN2', responseObject);
    ...
    async savePendingFeedback(data) {
        try{
            ...
            //checks if each object of the array contains all the required parameters
            data.forEach(x => {
                let [paramsValidated, err] = this.restaurantUtils.validateSavePendingFeedbackParams(x);
                if (!paramsValidated){
                    throw new Exception(STATUS_CODES.BAD_REQUEST, err)
                }
            });
            ...
            const feedbacks = restaurantModel.saveFeedback(feedbackArray);
    ...
    async savePendingFeedbackAgain(data) {
        try{
            ...
            //checks if each object of the array contains all the required parameters
            data.forEach(x => {
                let [paramsValidated, err] = this.restaurantUtils.validateSavePendingFeedbackAgainParams(x);
                if (!paramsValidated){
                    throw new Exception(STATUS_CODES.BAD_REQUEST, err)
                }
            });
            ...
            for (const x of data){
                ...
                const feedbacks = await restaurantModel.updateFeedback(x.orderID, x.itemID, feedback);
                feedbackArray.push(feedbacks);
            }
    ...
    async getPendingFeedback(data) {
        try{

           let [paramsValidated, err] = this.restaurantUtils.validateGetPendingFeedbackParams(data);
            if (!paramsValidated){
                throw new Exception(STATUS_CODES.BAD_REQUEST, err)
            }

            const feedbacks = await restaurantModel.getPendingFeedback({userID: data.userID});
    ...
    async updateQueue(data) {
        try{
           ...
           let [paramsValidated, err] = this.restaurantUtils.validateUpdateQueueParams(data);
            if (!paramsValidated){
                throw new Exception(STATUS_CODES.BAD_REQUEST, err)
            }
